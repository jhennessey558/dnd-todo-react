...
function Section({ status, tasks, setTasks, todos, inProgress, done }) {
    ...
    const bg = status === 'todo' ? 'bg-blue-500' : status === 'progress' ? 'bg-yellow-500' : 'bg-green-500'
...
function Header({ text, bg, count }) {
    return (
        <div className={`${bg} flex items-center h-10 pl-4 rounded-md uppercase text-sm text-white`}>
            {text}{" "}
            <div className="ml-2 bg-white rounded-full w-6 h-4 flex items-center justify-center text-black">
                {count}
            </div>
        </div>
    )
...
function Task({ task, tasks, setTasks }) {
    ...
    return (
        <div ref={drag} className={
            `relative flex items-center bg-zinc-200  h-4 p-4 mt-4 shadow-md rounded-lg cursor-grab ${isDragging ? 'opacity-50' : 'opacity-100'}`
        }>
